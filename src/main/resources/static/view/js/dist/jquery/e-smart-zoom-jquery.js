...
(function( $ ){
	 ...
	 */
		...
		if (publicMethods[method] ) { // if the parameter is an existing method, then we call this method
			...
			var tag = $(targetElement[0]).prop('tagName') || '';
			if(tag.toLowerCase() == "img" && !targetElement[0].complete){ // if the target is an image when wait for image loading before initialization
